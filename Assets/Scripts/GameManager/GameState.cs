...
public class GameState : AState
{
    ...
    public RectTransform questionMenu; // m
    ...
    protected int m_PreviousLife = 3; // m

    public QuestionPicker Picker; // m
    public Question question; // m
    protected bool playerWrongAnswer = false; //m
    ...
    public void StartGame()
    {
        ...
        // Initialize QuestionPicker & populate it
        Picker = new QuestionPicker();

        if (!trackManager.isRerun)
        {
            m_TimeSinceStart = 0;
            trackManager.characterController.currentLife = trackManager.characterController.maxLife;
            m_PreviousLife = trackManager.characterController.currentLife;
        }
    ...
    public override void Tick()
    {
        ...
        if (chrCtrl.currentLife != m_PreviousLife)
        {
            // Update current life
            m_PreviousLife = chrCtrl.currentLife;
            PauseForQuestion();
        }

        // New end state
        if (playerWrongAnswer == true)
        {
            playerWrongAnswer = false;
        ...
        // ------------ THIS IS HOW IT WAS PLAYED WITH LIVES --------------------
    ...
    {
        question = Picker.GetRandomQuestion();

        var formatQuestion = $"{question.number1.ToString()} " + $"{question.op}"  +  $" {question.number2.ToString()} = ?";
        var formatAnswer1 = $"{question.answer1.ToString()}";
        var formatAnswer2 = $"{question.answer2.ToString()}";
        var formatAnswer3 = $"{question.answer3.ToString()}";

        questionText.SetText(formatQuestion);
        firstAnswerText.SetText(formatAnswer1);
        secondAnswerText.SetText(formatAnswer2);
        thirdAnswerText.SetText(formatAnswer3);
    }
    ...
    public void QuestionAnswered(int buttonIndex)
    {
        ...
        // Player gave the wrong answer
        if ((result == question.answer1 && buttonIndex != 1) ||
            (result == question.answer2 && buttonIndex != 2) ||
            (result == question.answer3 && buttonIndex != 3))
        {
            playerWrongAnswer = true;
        }
        // In any case Resume()
        Resume();
    }
